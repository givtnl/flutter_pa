...
enum _CustomSliderType { material, adaptive }
...
class CustomSlider extends StatefulWidget {
  ...
  }) : _sliderType = _CustomSliderType.material,
        assert(value != null),
        assert(min != null),
        assert(max != null),
        assert(min <= max),
  ...
  /// Creates an adaptive [CustomSlider] based on the target platform, following
  /// Material design's
  ...
  const CustomSlider.adaptive({
    ...
    this.focusNode,
    this.autofocus = false,
  }) : _sliderType = _CustomSliderType.adaptive,
        assert(value != null),
        assert(min != null),
        assert(max != null),
        assert(min <= max),
  ...
  final _CustomSliderType _sliderType ;
...
class _CustomSliderState extends State<CustomSlider> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    _shortcutMap = <LogicalKeySet, Intent>{
      LogicalKeySet(LogicalKeyboardKey.arrowUp): const _CustomAdjustSliderIntent.up(),
      LogicalKeySet(LogicalKeyboardKey.arrowDown): const _CustomAdjustSliderIntent.down(),
      LogicalKeySet(LogicalKeyboardKey.arrowLeft): const _CustomAdjustSliderIntent.left(),
      LogicalKeySet(LogicalKeyboardKey.arrowRight): const _CustomAdjustSliderIntent.right(),
    };
    _actionMap = <Type, Action<Intent>>{
      _CustomAdjustSliderIntent: CallbackAction<_CustomAdjustSliderIntent>(
  ...
  void _actionHandler(_CustomAdjustSliderIntent intent) {
    final _CustomRenderSlider renderSlider = _renderObjectKey.currentContext!.findRenderObject()! as _CustomRenderSlider;
    final TextDirection textDirection = Directionality.of(_renderObjectKey.currentContext!);
    switch (intent.type) {
      case _CustomSliderAdjustmentType.right:
        ...
        break;
      case _CustomSliderAdjustmentType.left:
        switch (textDirection) {
          case TextDirection.rtl:
            renderSlider.increaseAction();
            break;
        ...
        break;
      case _CustomSliderAdjustmentType.up:
        renderSlider.increaseAction();
        break;
      case _CustomSliderAdjustmentType.down:
        renderSlider.decreaseAction();
        break;
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    switch (widget._sliderType) {
      case _CustomSliderType.material:
        return _buildMaterialSlider(context);

      case _CustomSliderType.adaptive: {
    ...
    return Semantics(
      ...
      child: FocusableActionDetector(
        ...
        child: CompositedTransformTarget(
          link: _layerLink,
          child: _CustomSliderRenderObjectWidget(
...
class _CustomSliderRenderObjectWidget extends LeafRenderObjectWidget {
  const _CustomSliderRenderObjectWidget({
    Key? key,
    required this.value,
  ...
  @override
  _CustomRenderSlider createRenderObject(BuildContext context) {
    return _CustomRenderSlider(
      value: value,
      divisions: divisions,
      label: label,
      sliderTheme: sliderTheme,
  ...
  @override
  void updateRenderObject(BuildContext context, _CustomRenderSlider renderObject) {
    renderObject
...
class _CustomRenderSlider extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  _CustomRenderSlider({
  ...
  static const Duration _minimumInteractionTime = Duration(milliseconds: 10000);
...
class _CustomAdjustSliderIntent extends Intent {
  const _CustomAdjustSliderIntent({
    required this.type,
  });

  const _CustomAdjustSliderIntent.right() : type = _CustomSliderAdjustmentType.right;

  const _CustomAdjustSliderIntent.left() : type = _CustomSliderAdjustmentType.left;

  const _CustomAdjustSliderIntent.up() : type = _CustomSliderAdjustmentType.up;

  const _CustomAdjustSliderIntent.down() : type = _CustomSliderAdjustmentType.down;

  final _CustomSliderAdjustmentType type;
}

enum _CustomSliderAdjustmentType {
  right,
  left,
  up,
  down,
...
class _ValueIndicatorRenderObjectWidget extends LeafRenderObjectWidget {
  ...
  @override
  _CustomRenderValueIndicator createRenderObject(BuildContext context) {
    return _CustomRenderValueIndicator(
      state: state,
    );
  }
  @override
  void updateRenderObject(BuildContext context, _CustomRenderValueIndicator renderObject) {
    renderObject._state = state;
  }
}

class _CustomRenderValueIndicator extends RenderBox with RelayoutWhenSystemFontsChangeMixin {
  _CustomRenderValueIndicator({
    required _CustomSliderState state,
  }) : _state = state {
    _valueIndicatorAnimation = CurvedAnimation(
      parent: _state.valueIndicatorController,
